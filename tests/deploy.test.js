...
describe('Deploying a CF app to IBM Cloud', () => {
  it('runs the right set of commands', async () => {
    ...
    await deploy();
    expect(exec).toHaveBeenCalledWith('ibmcloud', ['cf', 'push', 'cf-app-foo', '--strategy', 'rolling'], execOptions);
  });
  ...
  it('runs the right set of commands with deploy directory specified', async () => {
    ...
    await deploy();
    expect(exec).toHaveBeenCalledWith('ibmcloud', ['cf', 'push', 'cf-app-foo', '--strategy', 'rolling'], {
      ...execOptions,
      cwd: 'deploy-dir-foo',
    });
  });
